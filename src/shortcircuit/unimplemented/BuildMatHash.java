...
public class BuildMatHash implements Runnable {
    ...
    public BuildMatHash(GraphicsState gs, String[] towerTypes,
            Object[] creepTypes) {
        this.gs = gs;
    ...
    public void run() {
        for (int i = 0; i < towerTypes.length; i++) {
            matHash.put("Tower"+towerTypes[i], 
                    assetManager.loadMaterial(gs.getMatLoc(
                    "Tower"+towerTypes[i])));
        }
        matHash.put("Bomb", assetManager.loadMaterial(gs.getMatLoc("Bomb")));
        matHash.put("CreepSpawner", assetManager.loadMaterial(gs
                .getMatLoc("CreepSpawner")));
        ...
        for (int i = 1; i < 5; i++) {
            matHash.put("Tower"+i+"Beam", assetManager.loadMaterial(gs
                    .getMatLoc("Tower"+i+"Beam")));
        }
        for (int i = 0; i < creepTypes.length; i++) {
            matHash.put(creepTypes[i]+"Creep", assetManager.loadMaterial(gs
                    .getMatLoc(creepTypes[i]+"Creep")));
        }
    }
